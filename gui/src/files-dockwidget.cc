  ...
  : QDockWidget (parent)
{
  ...
  _directory_icon = QIcon(style->standardIcon (QStyle::SP_FileDialogToParent));
  ...
  if (fileInfo.isDir ())
    {
      ...
      set_current_directory (_file_system_model->fileInfo (index).
                           absoluteFilePath ());
      emit change_working_directory (_file_system_model->fileInfo (index).
                           absoluteFilePath ());
    }
...
files_dock_widget::do_up_directory (void)
{
  ...
  emit change_working_directory (dir.absolutePath ());
}
  ...
  if (fileInfo.isDir ())
    {
      ...
      emit change_working_directory (fileInfo.absoluteFilePath());
    }
  else
    {
...
files_dock_widget::notice_settings ()
{
  ...
  QFont font = QFont();
  if (settings->value ("filebrowser/defaultFont").toBool())
  {
  ...
  {    
    font.setFamily(settings->value("filebrowser/fontName").toString());
    font.setPointSize(settings->value("filebrowser/fontSize").toInt ());
  }
...
void
files_dock_widget::on_working_dir_changed(QString dirname)
{
  if (_current_directory->text() != dirname)
  {
    _file_system_model->setRootPath (dirname);
    _file_tree_view->setRootIndex (_file_system_model->index (dirname));

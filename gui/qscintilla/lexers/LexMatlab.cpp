...
static std::vector<std::string> _identifiers;
static std::vector<int> _identifiers_lines;
...
static void ColouriseMatlabOctaveDoc(
	...
	// boolean for when there is a = in the line
	...
	for (; sc.More(); sc.Forward(), column++) {
			...
			std::string exp;
			std::regex rexp("^[A-Za-z]+[A-Za-z0-9]*[ ]*=.*$", std::regex::basic);
			bool match = std::regex_match (target, rexp);

			if(match) {
			  std::cout << "Assignment: " << target << std::endl;
			  // If it is an assignment, add it to the list
			  _identifiers.insert(_identifiers.begin(), target);
			  _identifiers_lines.insert(_identifiers_lines.begin(), curLine);
			}*/
		...
		if (sc.state == SCE_MATLAB_OPERATOR) {
			if (sc.chPrev == '.') {
				if (sc.ch == '*' || sc.ch == '/' || sc.ch == '\\' || sc.ch == '^') {
					...
					sc.SetState(SCE_MATLAB_DEFAULT);
				        ...
				        sc.ForwardSetState(SCE_MATLAB_DEFAULT);
					// Check if assignment is true (the keyword is the identifier)
					  //std::cout << "Assigment! Var is " << varname << " and line is " << curLine << std::endl;
					  _identifiers.insert(_identifiers.begin(), varname);
					  _identifiers_lines.insert(_identifiers_lines.begin(), curLine);
					  varname.clear();
			...
			if (!isalnum(sc.ch) && sc.ch != '_') {
				...
				{
				    sc.ChangeState(SCE_MATLAB_KEYWORD_API);
				    sc.SetState(SCE_MATLAB_DEFAULT);
				    transpose = false;
				    assignment = false;
				} else {
				    ...
				    varname.clear();
				    varname.append(s);
				    // If varname is not in the assignment list, then put in red
				}
			}
		...
		} else if (sc.state == SCE_MATLAB_OCTAVE_COMMENT) {
			if (commentDepth == 0) {
				// single line comment Octave style comment
				if (sc.atLineEnd || sc.ch == '\r' || sc.ch == '\n') {
					sc.SetState(SCE_MATLAB_DEFAULT);
					transpose = false;
					assignment = false;
		...
		if (sc.state == SCE_MATLAB_DEFAULT) {
			...
			} else if (IsOctaveCommentChar(sc.ch)) {
				curLine = styler.GetLine(sc.currentPos);
				styler.SetLineState(curLine, commentDepth);
				sc.SetState(SCE_MATLAB_OCTAVE_COMMENT);

...
protected:
  void paintEvent (QPaintEvent* event) { q->viewPaintEvent (this, event, parentWidget()->hasFocus ()); }
...
QWinTerminalImpl::QWinTerminalImpl (const QString& cmd, QWidget* parent)
    : QTerminalInterface (parent), d (new QConsolePrivate (this, cmd)), _ctrlPressed(false)
{
}
...
bool QWinTerminalImpl::winEvent (MSG* msg, long* result)
{
  ...
  //BOOL altgr  = (GetAsyncKeyState (VK_RMENU)   < 0) | (GetAsyncKeyState (VK_MENU) < 0) | GetAsyncKeyState(VK_LMENU);
  BOOL f5 = (GetAsyncKeyState (VK_F5)   < 0);
  BOOL f6 = (GetAsyncKeyState (VK_F6)   < 0);
  BOOL f10 = (GetAsyncKeyState (VK_F10)   < 0);
  BOOL f11 = (GetAsyncKeyState (VK_F11)   < 0);
  //BOOL dk   = (GetAsyncKeyState (0x44)     < 0);
  //BOOL l      = (GetAsyncKeyState (0x4C)   < 0);
  //BOOL k      = (GetAsyncKeyState (0x4B)   < 0);
...
{
  d->m_font = f;
  d->m_font.setFamily (f.family());
  d->m_font.setPointSize (f.pointSize());
  d->m_font.setStyleHint (f.styleHint());
